...
describe('EmployeeObservationsController', () => {
  let controller: EmployeeObservationsController;
  let service: jest.Mocked<EmployeeObservationsService>;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        { provide: EmployeeObservationsService, useValue: mockService },
      ],
    })
    ...
    controller = module.get<EmployeeObservationsController>(
  ...
  it('should create observation', async () => {
    ...
    const result = await controller.create(mockCurrentUser, 'employee-1', {
      observation: 'Test',
    });
  ...
  it('should get all observations', async () => {
    service.findAllByEmployeeId.mockResolvedValue([mockObservation]);
    const result = await controller.findAllByEmployeeId(
      mockCurrentUser,
      'employee-1',
    );
    expect(result).toEqual([mockObservation]);
