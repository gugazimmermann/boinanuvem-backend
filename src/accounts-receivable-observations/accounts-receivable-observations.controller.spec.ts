...
describe('AccountsReceivableObservationsController', () => {
  let controller: AccountsReceivableObservationsController;
  let service: jest.Mocked<AccountsReceivableObservationsService>;
  ...
  beforeEach(async () => {
    const mockService = {
      create: jest.fn(),
      findAll: jest.fn(),
      findAllByAccountsReceivableId: jest.fn(),
      findOne: jest.fn(),
      update: jest.fn(),
      remove: jest.fn(),
    };
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        {
          provide: AccountsReceivableObservationsService,
    ...
    controller = module.get<AccountsReceivableObservationsController>(
      AccountsReceivableObservationsController,
    );
  ...
  it('should get observations by accounts receivable id', async () => {
    service.findAllByAccountsReceivableId.mockResolvedValue([mockObservation]);
    const result = await controller.findAllByAccountsReceivableId(
      mockCurrentUser,
      'ar-1',
    );
    expect(result).toEqual([mockObservation]);
