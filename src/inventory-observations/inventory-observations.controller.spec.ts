...
describe('InventoryObservationsController', () => {
  let controller: InventoryObservationsController;
  let service: jest.Mocked<InventoryObservationsService>;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        { provide: InventoryObservationsService, useValue: mockService },
      ],
    })
    ...
    controller = module.get<InventoryObservationsController>(
      InventoryObservationsController,
    );
  ...
  it('should create observation', async () => {
    ...
    const result = await controller.create(mockCurrentUser, 'item-1', {
      observation: 'Test',
    });
  ...
  it('should get all observations', async () => {
    service.findAllByItemId.mockResolvedValue([mockObservation]);
    const result = await controller.findAllByItemId(mockCurrentUser, 'item-1');
    expect(result).toEqual([mockObservation]);
  });
