...
describe('InventoryObservationsService', () => {
  let service: InventoryObservationsService;
  let prismaService: jest.Mocked<PrismaService>;

  const mockUser = { id: 'user-1', companyId: 'company-1' };
  const mockItem = { id: 'item-1', companyId: 'company-1', deletedAt: null };
  ...
  const mockCreateDto: CreateInventoryObservationDto = {
    observation: 'Test observation',
    fileIds: ['file-1'],
  };
  ...
  beforeEach(async () => {
    ...
    service = module.get<InventoryObservationsService>(
      InventoryObservationsService,
    );
  ...
  describe('create', () => {
    it('should create successfully', async () => {
      ...
      prismaService.inventoryItem.findFirst.mockResolvedValue(mockItem);
      prismaService.inventoryObservation.create.mockResolvedValue(
        mockObservation,
      );

      const result = await service.create('user-1', 'item-1', mockCreateDto);
    ...
    it('should throw if item not found', async () => {
      ...
      await expect(
        service.create('user-1', 'item-1', mockCreateDto),
  ...
  describe('findAllByItemId', () => {
    it('should return observations', async () => {
      prismaService.user.findUnique.mockResolvedValue(mockUser);
      prismaService.inventoryItem.findFirst.mockResolvedValue(mockItem);
      prismaService.inventoryObservation.findMany.mockResolvedValue([
        mockObservation,
      ]);

      const result = await service.findAllByItemId('user-1', 'item-1');
  ...
  describe('findOne', () => {
    it('should return observation', async () => {
      prismaService.user.findUnique.mockResolvedValue(mockUser);
      prismaService.inventoryObservation.findFirst.mockResolvedValue(
        mockObservation,
      );
  ...
  describe('update', () => {
    it('should update successfully', async () => {
      const updateDto: UpdateInventoryObservationDto = {
        observation: 'Updated',
      };
      prismaService.user.findUnique.mockResolvedValue(mockUser);
      prismaService.inventoryObservation.findFirst.mockResolvedValue(
      ...
      prismaService.inventoryObservation.update.mockResolvedValue({
        ...mockObservation,
        observation: 'Updated',
      });
